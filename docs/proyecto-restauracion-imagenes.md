...
**Límite honesto de la app en texto (medición):** la web app reutiliza el mecanismo del baseline de §6.8.2, no vuelve a medir sobre el dataset de validación (que vive en Drive). La verificación local de la ruta de texto usa un fixture sintético (screenshot de texto generado con PIL y degradado con JPEG), no el set real de la memoria. **Restricción de despliegue (Render free):** Render bloquea `apt-get` (root en solo lectura en build y en runtime), así que `render.yaml` empaqueta Tesseract como binario **estático** (`DanielMYT/tesseract-static` v5.5.3, `tesseract.x86_64`) + `eng.traineddata` de `tessdata_fast` en `vendor/` (gitignored), y `pipeline.py` lo detecta y apunta `pytesseract.tesseract_cmd` + `TESSDATA_PREFIX`; local/Colab siguen usando el Tesseract del sistema. Nota de honestidad: el `eng.traineddata` de `tessdata_fast` no es idéntico al del paquete de sistema con el que se validó §6.8.2 — el clásico en Render usa un motor y datos ligeramente distintos, sin re-validación formal del set de Drive.

**Selección de la mejor lectura de texto (medición en caso real, N=1 — no estadística):** la app solía detenerse en el **primer** método que devolvía texto (umbral de confianza 50), y en una imagen real de un poema (screenshot 739×415, texto pequeño) eso mostraba "original" (similitud normalizada 0.653 vs. el texto real). Al probar **todos** los métodos y medir contra el texto del poema, **Otsu ×4** lee mejor (**0.768** con Tesseract) que el original (0.653) y que upscale ×4 (0.672). **Hallazgo medible clave:** la confianza media de Tesseract **no** predice la utilidad funcional — upscale ×4 da conf 83.5 con ratio 0.672 mientras que Otsu ×4 da conf 81.1 (peor en **todas** las señales de confianza: media, mediana, p25, p10) y aun así lee mejor. Es el mismo patrón ya documentado para PSNR/SSIM aplicado a la confianza del OCR: la señal de píxel no manda, el contenido sí. **Segundo hallazgo (misma imagen):** la confianza de **RapidOCR es plana** — da ~0.95-0.98 tanto a lecturas buenas como malas (0.981 para una lectura con ratio 0.18), por lo que no sirve para ordenar métodos ni para parada temprana; su lectura sí es mejor cuando el contenido es bueno. Diseño resultante (lector `auto`, el defecto): el clásico Tesseract se prueba primero y **puede** detenerse si lee confiado (conf ≥ 90) o exacto contra `expected`; si no convence, se prueba RapidOCR y se selecciona la mejor lectura de **ambos** — con `expected` por similitud funcional (`select_best_match`), sin él por banda de confianza + la más larga (`select_best_read`, `CONF_BAND=3.0`, confianza de Rapid normalizada ×100). En el poema el resultado pasa de 0.653 (primera lectura) a **0.802** (RapidOCR Otsu ×4), con mucho mejor espaciado (menos palabras pegadas). Coste medido: ~23-41 s en CPU local para este caso degradado (ambos lectores); en texto limpio el clásico se detiene rápido. Con `expected`=poema el resultado sigue siendo **`false_positive`** (0.802 < 0.9): el OCR lee casi todo pero con errores (tildes, `ytodo` por `y todo`, `trobajan` por `trabajan`) — la verificación lo reporta con honestidad, no lo esconde.
